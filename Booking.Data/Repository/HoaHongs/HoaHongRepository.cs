...
namespace Booking.Data.Repository.HoaHongs
{
    internal class HoaHongRepository : IHoaHongRepository
    {
        ...
        public async Task<IEnumerable<HoaHong>> LayHoaHongChoThu(Guid ksId, DateTime tuNgay, DateTime denNgay)
        {
            var hoaHongs = await _context.HoaHongs
                    .Where(x =>
                        x.MaKhachSan == ksId &&
                        x.TrangThai == TrangThaiHoaHong.CHO_THU.ToString() &&
                        x.NgayTao >= tuNgay &&
                        x.NgayTao < denNgay.AddDays(1))
                    .ToListAsync();
            return hoaHongs;
        }
    }
